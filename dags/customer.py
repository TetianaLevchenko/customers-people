...
def _read_customers():
    ...
    with open('/opt/data/customers-100.csv', "rt") as file_obj:
        ...
        target_fields = ['Index', 'Customer_Id', 'First_Name', 'Last_Name', 'Company', 'City', 'Country', 'Phone_1', 'Email', '"Subscription Date"', 'Website']

        hook.insert_rows(table='customers', rows=rows, target_fields=target_fields, commit_every=1000, replace=True, replace_index="index")
        
        
def _save_customers_to_excel():
    hook = PostgresHook(postgres_conn_id="postgres")
    customers_file = '/opt/data/customers.xlsx'
    ...
    df = hook.get_pandas_df(sql="SELECT * FROM (SELECT DISTINCT ON (customer_id) * FROM customers ) AS unique_customers ORDER BY index")
    df['Years_Until_Subscription'] = df['Subscription Date'].apply(years_until)
